...
//---------------------------------------------------------------------------
__fastcall TFormLognormal::TFormLognormal(TComponent* Owner)
    : TForm(Owner)
{
...
void __fastcall TFormLognormal::Button1Click(TObject *Sender)
{
	try
	{
		...
		// Unico parâmetro que muda em relação à distribuição Normal padrão
		x = logf( x );
